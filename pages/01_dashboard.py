...
# --- CSS EXECUTIVO: DESIGN HORIZONTAL E LEGIBILIDADE ---
st.markdown("""
    <style>
...
def fetch_dashboard_data(token):
    ...
    hierarquia = [
        "RECEITA MAO DE OBRA", "RECEITA LOCAÇÃO", "RECEITA DE INDENIZAÇÃO",
        "CUSTO COM MAO DE OBRA", "CUSTO COM INSUMOS", "LOCAÇÃO DE EQUIPAMENTOS"
    ]
...
# 5. SIDEBAR - FILTROS HIERÁRQUICOS
...
# FILTRO DE MESES LIMPO
lixo = ["Revisão", "Cenário", "Semana", "Observações:", "ID", "Classificação", "Gerência", "Complexo", "Área", "Análise de emissão", "CC", "DataHora"]
todos_meses = [c for c in df_raw.columns if c not in lixo and not str(c).startswith("Unnamed")]
sel_meses = st.sidebar.multiselect("Período Analisado", todos_meses, default=todos_meses[:6] if len(todos_meses) >= 6 else todos_meses)
...
fig_bar = px.bar(
    df_agrupado, x="Mês", y="Valor", color="Revisão", barmode="group", text_auto='.2s',
...
st.dataframe(df_pivot_abs[sel_rev_geral].style.format("R$ {:,.2f}"), use_container_width=True)

# 9. MONTAGEM DE COMPARATIVOS (DELTAS) - CORRIGIDO
st.markdown('<div class="section-header">⚖️ Laboratório de Variações (Montagem de Deltas)</div>', unsafe_allow_html=True)

with st.container():
    st.markdown('<div class="delta-selector-card"><strong>🛠️ Configurar Comparativos</strong><br>Selecione as semanas disponíveis no filtro lateral para montar os pares de variação.</div>', unsafe_allow_html=True)
    
    if 'comparativos' not in st.session_state:
        st.session_state.comparativos = [{"id": 0}]
    ...
    for i, comp in enumerate(st.session_state.comparativos):
        ...
        # Filtramos as opções dos selects para apenas as semanas que existem no df_pivot_abs
...
if not df_deltas_final.empty:
    ...
    fig_line = px.line(
        df_deltas_final.reset_index().melt(id_vars="Mês", var_name="Comparativo", value_name="Variação"),
        x="Mês", y="Variação", color="Comparativo", markers=True,
        title="Tendência das Variações Selecionadas",
    ...
    st.dataframe(df_deltas_final.style.format("R$ {:,.2f}").applymap(color_delta), use_container_width=True)

# 11. DETALHAMENTO FINAL
st.markdown('<div class="section-header">🔍 Detalhamento por Conta (Análise de Emissão)</div>', unsafe_allow_html=True)
semanas_vivas = list(set([c.get('semana_a') for c in st.session_state.comparativos if c.get('semana_a')] + 
                         [c.get('semana_b') for c in st.session_state.comparativos if c.get('semana_b')]))
